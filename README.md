...
- **证券代码研究报告链路**：新增证券代码分析模式和 `/api/stock-reports` SSE 入口，支持普通 A 股和常见 ETF，按代码解析、数据快照、指标计算、风险评分、证据账本、报告撰写、引用审查生成八章节研究报告，并统一标注“不构成投资建议”。
...
- **TuShare 授权行情数据源**：新增 `TushareMarketDataProvider`，可通过环境变量接入 TuShare Pro，将普通股票的利润表、资产负债表、现金流和日行情基础指标转为金融证据账本输入；ETF 优先使用 `fund_daily` 映射收盘价、涨跌幅和成交额。
...
- `ticker`：普通 A 股或常见 ETF 的 6 位代码，也支持带 `.SH` / `.SZ` 后缀的代码。普通 A 股支持沪市 `6xxxxx`、深市 `0xxxxx / 2xxxxx / 3xxxxx`；ETF 支持沪市 `5xxxxx`、深市 `15xxxx / 16xxxx / 18xxxx`。
- `report_period`：报告期口径，默认 `latest`。
- `search_mode`：复用 `document`、`hybrid`、`web`；MVP 不引入 Python、akshare、OpenSearch 或 pgvector。
- `thread_id`：可选；不传时后端会生成证券研究报告线程 ID。
