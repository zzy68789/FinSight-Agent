...
- **A股投研报告链路**：新增股票代码分析模式和 `/api/stock-reports` SSE 入口，按股票解析、数据快照、指标计算、证据账本、报告撰写、引用审查生成八章节 A 股投研报告，并统一标注“不构成投资建议”。
...
```text
StockResolve -> DataSnapshot -> MetricEngine -> EvidenceCollect -> InvestmentWriter -> CitationReviewer -> END
```
