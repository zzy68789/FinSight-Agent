        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement statement = connection.prepareStatement("""
                    INSERT INTO stock_analysis_snapshot(
                      ...
                      report_period, search_mode, snapshot_json, status, created_at, updated_at
                    )
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        for (FinancialMetricResult metric : metrics) {
            jdbcClient.sql("""
                            INSERT INTO stock_metric_result(
                              snapshot_id, task_id, metric_name, formula, metric_value, display_value,
                              status, reason, evidence_refs, created_at
                            )
                            VALUES (:snapshotId, :taskId, :metricName, :formula, :metricValue, :displayValue,
