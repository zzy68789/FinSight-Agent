...
public class TushareMarketDataProvider implements FinancialDataProvider {
    ...
    public TushareMarketDataProvider(
            ...
            @Value("${drai.market.tushare.enabled:false}") boolean enabled,
            @Value("${drai.market.tushare.base-url:https://api.tushare.pro}") String baseUrl,
            @Value("${drai.market.tushare.api-key:}") String token,
            @Value("${drai.market.tushare.timeout:10s}") String timeout
    ) {
